...
> Added statement based on our discussions in the viva to end of
> _Research Contributions_: namely that it is hard, but that if AppPAL
> manages to capture the policies, and what we learn from doing so
> (and the surrounding discussion) is interesting then the work is a
> success.
...
> N.B. This chapter was integrated into Ch 2. (as per 7.2)
