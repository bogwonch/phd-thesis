...
rule (we have \fade{grayed out} the recursive case), hence we would
...
\paragraph*{Inductive Step.}  For a terminating, assuming the base
case is correct and \texttt{Evaluate} is sound without recursion, we
must argue that \texttt{Evaluate} in the non-recursive case also.
For the non-terminating case, Evaluate will either return \texttt{Failure} (in the case of a circular argument) or not return if the size of the policy is infinite.
...
also return \texttt{Success.}

\paragraph{Induction Hypothesis.}  Suppose know that for any proof
tree for a query $q$ (given an $AC$) of height $n$, then running
\texttt{Evaluate} with that query and $AC$ will return
\texttt{Success} (with at least $n - 1$ recursive calls to
\texttt{Evaluate}).  Our hypothesis is that for a proof tree of height
$n+1$, then \texttt{Evaluate} will also return success (given the same
$AC$ and $q$), with at least $n$ recursive calls to \texttt{Evaluate}.
...
that these will be complete and will have at most $n-1$ recursive
...
able to evaluate the corresponding assertions and with recursive depth
$n$ (the height of the proof tree minus the current call).
