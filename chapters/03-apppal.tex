...
\emph{cond} rule (shown below with recursive elements \fade{grayed
...
root node. To show the hypothesis is true we must show that the
however the \texttt{Evaluate} function proceeds must correspond to one
of the AppPAL's evaluation rules, so the corresponding proof tree
...
%\paragraph*{Inductive Step.}  For a terminating query, assuming the base
