...
(i.e. the unifying assertion has no conditions) and the constraint
...
could satisfy its conditions. If there are no facts that could ever
satisfy the rule, then the policy may have a bug. An analogy can be
drawn to a \emph{conditional always false bug} in a conventional
...
\begin{align*}
  G \in \text{Satisfiable}~\text{\textit{if}}~\exists A \in \text{AC}:~&\exists \theta:~G\equiv\text{head}(A\theta) \\
                                                              \wedge~ & (\text{conditions}(A) = \emptyset \\
                                                                      & \vee~\forall G^\prime \in \text{conditions}(A\theta).~G^\prime \in \text{Satisfiable}.)
\end{align*}
