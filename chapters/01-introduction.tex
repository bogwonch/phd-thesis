...
%\begin{itemize}
%\item Mobile devices are ubiquitous.
%\item With increasing device power there has also been an increasing desire to
  %control them. From user's having preferences about what data the device should
  %access and which apps they want to use to companies seeking to control what data
  %each device can access.
%\item As part of the devices mobility there has been a move towards
  %decentralization. Users can install apps from many sources. Two users may share
  %different policies. Tools for running policy checks may not be run locally as
  %before, but on a distant server. Apps come with guarantees from stores that they
  %are safe to use.
%\item This distributed and decentralized ecosystem relies on trust relationships
  %to work. A user or company may not know precisely what checks the store is doing
  %on the apps they sell but they trust the store to make the judgement.
%\item Despite the prevalence of these trust relationships, the policies that
  %define them are frequently \emph{informal}. Either natural language documents,
  %unwritten preferences, or hidden in the code implementing the devices and
  %software.
%\item Existing research has focussed on the question of how can we enforce ever
  %more sophisticated policies, using ever more sophisticated techniques.
%\item This work takes a step back: it asks how we can express the informal
  %policies and trust relationships surrounding mobile devices formally.
%\item By using authorization languages we can model these policies precisely,
  %and start to analyse and make comparisons between them.
%\item This is the central question of this thesis: how can we use formal
  %languages to model the informal policies and trust relationships surrounding
