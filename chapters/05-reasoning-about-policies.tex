...
Analysing each of the policies common concerns become clear.
...
All policies had rules describing when to erase the device for security reasons (as shown in~\autoref{sec:overview-of-five-byod-policies}).
Many \ac{MDM} programs let an administrator remotely erase a device~(\autoref{tab:mdm-capabilities}).
...
AppPAL doesn't remove the need for
\ac{MDM} software.  Much of an \ac{MDM} package's functionality could be
re-implemented as part of AppPAL, but this is a massive duplication of work.
AppPAL works best when acting as a glue between existing tools.  It tells you
...
configuration language, but it also gives you more.  Giving employees a means to
