...
delegation and acknowledgements, that existing tools for enforcing BYOD
policies ignore.
...
BYOD policies.  To do this we analyse of five \emph{real-world} policies.
...
published by security advice organisations.  The remaining three are
...
Table~\ref{tab:mdm-capabilities} summarises the features of 9 competing \ac{MDM} packages identified by a Gartner
...
\emph{niche player}. The meaning of each term is unclear, however.\footnote{Table~\ref{tab:mdm-capabilities}
  summarises the visionary and leading \ac{MDM} packages.}  The tools are similar with the main difference being the UI
...
advanced than what can be written using a policy language such as AppPAL. The
...
become confused. 
%
To make the comparisons precise, and to identify common authorisation patterns in the polices we propose the method summarised in \autoref{tab:experiment-byod}: encode each of the natural language policies in AppPAL and then use the more formal AppPAL version as the basis for examining the policies.
...
employee's manager to authorise a decision (as in the NHS policy).  It
...
part of BYOD policies, many \ac{MDM} tools seem to largely to ignore it,
...
For example, an employee's manager must authorise an app install.
...
employee behaviour. An administrator cannot configure a device so that a user is
...
\vspace{1em}
The SANS, NHS and the Sirens company policies use acknowledgements to link to
...
We use acknowledgements when employees should be aware of (usually separate) rules.
