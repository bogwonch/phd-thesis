...
Whilst we do not know of any concurrent work modeling the informal policies and
trust relationships surrounding the mobile ecosystem; there has been work on
policy languages that is relevant to our work. In this chapter we give an
overview of DKAL which built on SecPAL to describe how information could be
shared. We also describe XACML an industrial standard for describing policies
that recently has been extended to describe delegation relationships and explain
...
The coarseness of Android's permission system and its relative inflexibility led
to a line of research into \emph{fine-grained} permissions systems. These
systems would add additional controls to permissions and allow greater control
about when they were enforced. We describe several notable examples and how each
added additional controls or provided new means to enforce the policies.
...
\begin{figure}
  ...
  \begin{mdframed}
    %\begin{multicols}{1}
      \lstinputlisting[basicstyle=\ttfamily\tiny,frame=none,breaklines=true]{figures/xacmldelegation.xml}
    %\end{multicols}
  \end{mdframed}
  \end{minipage}
