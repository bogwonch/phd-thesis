...
\title{On Formally Describing Informal Mobile Policies} 
\author{Joseph Hallett}
\submityear{2017} 
\abstract{ 

  This thesis looks at developing an authorization logic to describe the
informal policies in the mobile ecosystem. These policies are described using
natural language such as BYOD policies or terms and conditions, as privacy
preferences that users may never write down, or as trust relationships implicit
in the software running on mobile devices. Whilst prior work has looked on
checking and enforcing policies with low-level controls, this work aims to
capture the intent of the policies and the trust relationships within them
separating the specification of the policy from its enforcement. This allows us
to analyze the informal policies precisely, measure how often they are used in
practice, and look for idioms.

  We show how AppPAL can be instantiated from SecPAL and implemented as an
authorization logic for mobile ecosystems, and how we can check AppPAL policies
for common errors. Using AppPAL we show that policies describing users privacy
preferences do not seem to match the apps users install. We explore the
differences between app stores and how to create new ones based on policy. We
examine five BYOD policies and discover previously unexamined idioms within
them. 
